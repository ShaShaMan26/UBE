    ...
    public void generateBullets(Rectangle battleBox) {
        ...
        switch (rotation) {
            case 3 -> {
                ...
                bullets.add(new Bullet(damVal, battleBox.x + battleBox.width + x + sprite.getHeight(), y, bSpeed, fadeSpeed, rotation, sprite));
            }
            case 2 -> {
                ...
                bullets.add(new Bullet(damVal, x, battleBox.y - y - sprite.getHeight(), bSpeed, fadeSpeed, rotation, sprite));
            }
            case 1 -> {
                x = (int) (Math.random() * sprite.getHeight() * 2);
                y = (int) (Math.random() * ((battleBox.y + battleBox.height - sprite.getWidth()) - battleBox.y) + battleBox.y);
                bullets.add(new Bullet(damVal, battleBox.x - x - sprite.getHeight(), y, bSpeed, fadeSpeed, rotation, sprite));
            }
            default -> {
                x = (int) (Math.random() * ((battleBox.x + battleBox.width - sprite.getWidth()) - battleBox.x) + battleBox.x);
                y = (int) (Math.random() * sprite.getHeight() * 2);
                bullets.add(new Bullet(damVal, x, battleBox.y + battleBox.height + y, bSpeed, fadeSpeed, rotation, sprite));
            }
        }
    }
